...
import { clampRectRadii, roundSubPathCorners } from './corners';
...
export function rectSubPath(width: number, height: number, radii: [number, number, number, number] = [0, 0, 0, 0]): SubPath {
  // each corner may use its whole edges minus the neighbouring corner (a 47×61 body with a
  // 37 radius at one corner is fine); only pairs that overlap are scaled down
  const [tl, tr, br, bl] = clampRectRadii(width, height, radii);
  const k = 1 - KAPPA;
...
/**
 * Round every corner of a subpath with one radius (the Round Corners effect). Corners are
 * detected by the tangent break, so it also works on the dense smooth output of a warp.
 */
export function roundCorners(sp: SubPath, radius: number): SubPath {
  if (radius <= 0) return sp;
  return roundSubPathCorners(sp, () => radius);
}
