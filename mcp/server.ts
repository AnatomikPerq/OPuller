...
const idsSchema = z.array(z.string()).optional().describe('Node ids; defaults to the current selection. Ids that exist no more are skipped; when none of them exists the call fails (it never falls back to the selection)');
...
server.registerTool(
  ...
  {
    title: 'Blend',
    description: 'Object > Blend without the dialog. op make (default; ids or the selection, at least two paths, not already a blend) creates a blend group with spacing "steps" (steps count), "distance" (px between steps) or "smooth"; options changes an existing blend (error when the selection holds none); expand / release / reverse (spine) / reverseStack run the matching commands. Returns the blend group(s) with their child count (sources + steps).',
...
server.registerTool(
  'opuller_align',
  {
    ...
    description: 'Align panel from parameters (ids or the selection): h left | center | right, v top | middle | bottom, to selection (default) | artboard | key (key = id of the key object, else the topmost), distribute h | v (equal spacing between objects, spacing = px or "auto") or left | center | right | top | middle | bottom (equal distances between those edges / centres). Each step is one undo entry; "done" lists the steps that moved something (an already aligned selection gives []). The key object given here is used for this call only.',
...
server.registerTool(
  'opuller_set_appearance',
  {
    ...
    description:
      'Set the fill/stroke/text style. target "defaults" (default): only the current appearance that NEW objects (pencil, pen, brush, shapes, text) are drawn with — the selection is left alone. "selection": patch the selected objects (or ids) with the fields given, each object keeping the rest of its own stroke / fill (a stroke width alone keeps every colour). "both": selection and defaults, like editing the Color panel with a selection. Note: selecting an object makes its appearance current (Illustrator behaviour), so call this after selecting when the new object must differ.',
...
server.registerTool(
  'opuller_export_file',
  {
    ...
    description: 'Write the artboard / selection / document to disk as SVG, PDF, EPS or AI (format from the file extension or the format field). AI files are Illustrator 8 native (editable layers, text, gradients, spot colours) unless aiFormat is "pdf" (PDF-compatible .ai, opened by Illustrator as PDF content). text picks how AI text is written: auto / editable (text stays editable when it fits the file encoding, the rest is outlined) or outlines. encoding: auto (default — Windows-1251 when the text is mostly Cyrillic, Latin-1 otherwise), latin1 or cp1251; the result reports the encoding used. bleed/marks add the document bleed and printer marks; cmyk writes EPS / AI colours as inks.',
    inputSchema: { file: z.string(), format: z.enum(['svg', 'pdf', 'eps', 'ai']).optional(), aiFormat: z.enum(['legacy', 'pdf']).optional(), text: z.enum(['auto', 'editable', 'outlines']).optional(), encoding: z.enum(['auto', 'latin1', 'cp1251']).optional(), scope: z.enum(['artboard', 'artboards', 'selection', 'document']).optional(), ids: z.array(z.string()).optional(), artboardId: z.string().optional(), bleed: z.boolean().optional(), marks: z.boolean().optional(), cmyk: z.boolean().optional(), outlineText: z.boolean().optional() },
  },
  async (p) => {
    const abs = path.resolve(p.file);
    const format = p.format ?? (path.extname(abs).slice(1).toLowerCase() as 'svg' | 'pdf' | 'eps' | 'ai');
    const r = await call<{ text?: string; base64?: string; pages?: number; name?: string; encoding?: string; warnings?: string[]; failed?: string[] }>('exportFile', { ...p, format }, 120_000);
    await fs.mkdir(path.dirname(abs), { recursive: true });
    if (r.base64) await fs.writeFile(abs, Buffer.from(r.base64, 'base64'));
    else await fs.writeFile(abs, r.text ?? '', format === 'ai' ? 'latin1' : 'utf8');
    return text({ saved: abs, format, pages: r.pages, bytes: r.base64 ? Buffer.from(r.base64, 'base64').length : (r.text ?? '').length, encoding: r.encoding, warnings: r.warnings, failed: r.failed });
  },
);
