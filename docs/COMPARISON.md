...
* Warp (15 стилей, bend, Horizontal/Vertical distortion, обе оси) воспроизводит Illustrator
  ...
  расхождение решётки ≤ 0.1 px). Разбиение контура при Expand тоже как у Illustrator
  (`tests/fixtures/effects/warp-shapes.json`): прямые стороны вдоль осей — точные изокривые
  патча, любой другой отрезок — один кубик через образы четырёх точек (0, ⅓, ⅔, 1), который
  делится пополам, пока в четвертях отклоняется больше 0.2 px; треугольник после Expand даёт
  3 узла с ручками, ромб — 4, окружность — 8, как в Illustrator. Для кривых сторон ручки
  Illustrator отличаются на несколько px (его собственная аппроксимация), контур — в пределах ~1 px.
* Pucker & Bloat и Zig Zag сняты с Illustrator 2026 и совпадают с ним по узлам и ручкам
  ...
  80 случаев фикстуры совпадают в пределах 0.5 px. Transform (копии, опорная точка 3×3, порядок
  масштаб → поворот → сдвиг, накопление шага относительно исходной опорной точки) совпадает с
  Illustrator точно (`transform.json`, 70 случаев). Roughen случаен, но структура как у Illustrator:
  round(L·detail/72) кусков на сегмент (Illustrator считает точки на дюйм в 72 единицах), сдвиг
  каждой точки — включая узлы — вдоль нормали (в углу — по биссектрисе), в режиме Smooth ручки
  как у Zig Zag; сама случайная последовательность своя. Tweak не сверялся.
